...
export class KeyboardBuilder {
  ...
  static buildQuestionBoard(questionId, options) {
    ...
    for (let i = 0; i < list.length; i += 2) {
      const row = [];
      const o1 = list[i];
      const label1 = typeof o1 === 'string' ? o1 : o1.label || o1.name || `选项 ${i + 1}`;
      row.push(
        this.button({
          id: `opt_${i}`,
          label: label1.slice(0, 16),
          data: `/answer ${questionId} ${encodeURIComponent(label1)}`,
          style: i === 0 ? 1 : 0,
        })
      );
      ...
      if (i + 1 < list.length) {
        const o2 = list[i + 1];
        const label2 = typeof o2 === 'string' ? o2 : o2.label || o2.name || `选项 ${i + 2}`;
        row.push(
          this.button({
            id: `opt_${i + 1}`,
            label: label2.slice(0, 16),
            data: `/answer ${questionId} ${encodeURIComponent(label2)}`,
            style: 0,
          })
        );
      }
