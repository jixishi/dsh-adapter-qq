...
export class ApprovalHandler {
    ...
    this.disposer = this.ctx.on('approval/request', async (req, next) => {
      ...
      try {
        const raceResult = await Promise.race([
          ...
          new Promise((resolve) => {
            if (req.signal) {
              req.signal.addEventListener('abort', () => resolve({ source: 'abort', outcome: 'cancelled' }), {
                once: true,
              });
            }
          }),
        ...
        } else if (raceResult.source === 'qq') {
          this.logger.info?.(`[ApprovalHandler] Approval resolved from QQ: ${raceResult.outcome}`);
        }
  ...
  handleUserDecision(approvalId, decision) {
    // If no approvalId specified, but there is exactly 1 pending approval, use it
