...
export class SessionManager {
  ...
  async getSessionStats(sessionId) {
    ...
    const live = this.sessions?.get(sid);
    if (live && this.ctx.sessionProjections?.stateOf) {
      try {
        stats = this.ctx.sessionProjections.stateOf(live, 'sessionStats');
        tokenUsage = this.ctx.sessionProjections.stateOf(live, 'tokenUsage');
        modelSelection = this.ctx.sessionProjections.stateOf(live, 'modelSelection');
      } catch {
        // fallback
      }
    }
  ...
  async getModelCatalog() {
    ...
    // Current selection from agentDefaultModel
    if (this.ctx.agentDefaultModel?.currentSelection) {
      try {
        current = this.ctx.agentDefaultModel.currentSelection();
      } catch {
        // ignore
      }
    }
    ...
    if (this.sessionController?.modelCatalog) {
      try {
        ...
        if (catalog) {
          if (catalog.defaultSelection) current = catalog.defaultSelection;
    ...
    // 2. Fallback: parse settings.yaml if flatModels is empty
    if (flatModels.length === 0) {
      try {
        ...
        if (existsSync(settingsPath)) {
          ...
          const piAiMatch = raw.match(/llm-pi-ai:\s*([\s\S]*?)(?=\n[a-z0-9-]+:|\s*$)/i);
          if (piAiMatch) {
            const block = piAiMatch[1];
            const pMatches = [...block.matchAll(/([a-zA-Z0-9_-]+):\s*[\r\n]+\s+displayName:\s*([^\r\n]+)/g)];
            const mMatches = [...block.matchAll(/- id:\s*([a-zA-Z0-9._-]+)/g)];
            for (const m of mMatches) {
              flatModels.push({
                id: m[1],
                name: m[1],
                provider: pMatches[0]?.[1] || 'cpa',
                providerName: pMatches[0]?.[2] || 'CPA',
              });
            }
          }
        }
   ...
   * @returns {Promise<Object>}
   */
  async switchModel(modelName, provider) {
    ...
    // Support 1-based index
    const num = parseInt(modelName, 10);
    if (!isNaN(num) && num >= 1 && num <= models.length) {
      targetModel = models[num - 1];
    }

    const finalModel = targetModel ? targetModel.id : modelName;
    ...
    if (this.sessionController?.selectModel) {
      ...
      return res?.selected || { provider: finalProvider, model: finalModel };
    }
