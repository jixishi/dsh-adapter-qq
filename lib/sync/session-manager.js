...
export class SessionManager {
  ...
  /**
   * Create a new session and ensure it is attached to a workspace (not ungrouped)
   * @param {Object} options
  ...
  async createSession({ cwd, preset, sessionId, workspaceId } = {}) {
    ...
    // 1. If workspaceId given, resolve its path
    if (resolvedWsId && !resolvedCwd) {
      const ws = this.workspaceRegistry?.get?.(resolvedWsId);
      if (ws) resolvedCwd = ws.path;
    }

    // 2. Default cwd fallback
    if (!resolvedCwd) {
      const workspaces = this.workspaceRegistry?.list?.() || [];
      if (workspaces.length > 0) {
        resolvedCwd = workspaces[0].path;
        resolvedWsId = resolvedWsId || workspaces[0].id;
      } else {
        resolvedCwd = process.cwd();
      }
    }

    // 3. Find matching workspace if workspaceId not specified
    let targetWorkspace = resolvedWsId ? this.workspaceRegistry?.get?.(resolvedWsId) : undefined;
    if (!targetWorkspace && this.workspaceRegistry) {
      try {
        if (typeof this.workspaceRegistry.resolveByPath === 'function') {
          targetWorkspace = await this.workspaceRegistry.resolveByPath(resolvedCwd);
        }
      } catch {}

      if (!targetWorkspace && typeof this.workspaceRegistry.list === 'function') {
        const allWs = this.workspaceRegistry.list();
        targetWorkspace = allWs.find(
          (w) => w.path && w.path.toLowerCase().replace(/[\\/]+$/, '') === resolvedCwd.toLowerCase().replace(/[\\/]+$/, '')
        );
      }

      // If still no workspace exists for this directory, auto-create one
      if (!targetWorkspace && typeof this.workspaceRegistry.create === 'function') {
        try {
          targetWorkspace = await this.workspaceRegistry.create(resolvedCwd);
        } catch (e) {
          this.logger.warn?.(`[SessionManager] Auto create workspace failed: ${e.message}`);
        }
      }

      if (targetWorkspace) {
        resolvedWsId = targetWorkspace.id;
      }
    }

    this.logger.info?.(
      `[SessionManager] Creating session: id=${finalSessionId}, cwd=${resolvedCwd}, workspaceId=${resolvedWsId || '<none>'}, preset=${finalPreset}`
    );

    let createResult = null;

    if (this.sessionController?.create) {
      // If we have resolvedWsId, use workspaceId so DSH attaches it natively; otherwise cwd
      const req = resolvedWsId
        ? { sessionId: finalSessionId, workspaceId: resolvedWsId, agentPreset: finalPreset }
        : { sessionId: finalSessionId, cwd: resolvedCwd, agentPreset: finalPreset };

      createResult = await this.sessionController.create(req);
    } else if (this.sessions?.create) {
      const session = this.sessions.create(finalSessionId, {
        cwd: resolvedCwd,
        agentPreset: finalPreset,
      });
      createResult = { sessionId: session.id, agentPreset: finalPreset };
    } else {
      throw new Error('No session creation service available in DSH context.');
    }

    // 4. Guarantee attachment to workspace so it NEVER ends up ungrouped
    if (targetWorkspace && typeof targetWorkspace.attachSession === 'function') {
      try {
        await targetWorkspace.attachSession(finalSessionId);
        this.logger.info?.(`[SessionManager] Successfully attached session "${finalSessionId}" to workspace "${targetWorkspace.title}" (${targetWorkspace.id})`);
      } catch (err) {
        this.logger.warn?.(`[SessionManager] Failed to attach session to workspace: ${err.message}`);
      }
    }
    ...
    return {
      ...
      agentPreset: createResult?.agentPreset || finalPreset,
    };
  }
