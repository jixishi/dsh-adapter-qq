...
export class MessageBridge {
  ...
  async cmdModel(userOpenid, modelNameOrIndex, msgId) {
    ...
    if (modelNameOrIndex) {
      try {
        ...
        await this.apiClient.sendC2CMessage(userOpenid, {
          markdown: `🤖 模型切换成功！当前使用: **\`${selected.model}\`** (\`${selected.provider}\`)`,
          keyboard: KeyboardBuilder.buildCurrentSessionBoard(),
          msg_id: msgId,
        });
      } catch (err) {
        await this.apiClient.sendC2CMessage(userOpenid, {
          ...
          keyboard: KeyboardBuilder.buildModelsBoard(models, current?.model),
          msg_id: msgId,
        });
      }
      return;
    ...
    const lines = [
      '🤖 **【DSH 模型管理与切换】**',
      '',
      `当前会话模型: **\`${currentModelId}\`** (${current?.provider || 'default'})`,
      '',
      '**可用模型列表:**',
    ];

    models.slice(0, 10).forEach((m, idx) => {
      const isCur = m.id === currentModelId ? '✅ ' : '';
      lines.push(`${idx + 1}. ${isCur}**\`${m.id}\`** [${m.providerName || m.provider}]`);
    });

    lines.push('', '💡 请点击下方按钮切换模型，或回复 `/model <名称或序号>`：');

    await this.apiClient.sendC2CMessage(userOpenid, {
      markdown: lines.join('\n'),
      keyboard: KeyboardBuilder.buildModelsBoard(models, currentModelId),
      msg_id: msgId,
    });
  }
  ...
  async cmdHelp(userOpenid, msgId) {
    const lines = [
      ...
      '- `/model [名称]`: 查看或切换 AI 模型（如 gpt-5.6-luna / gemini-3.8-flash 等）',
