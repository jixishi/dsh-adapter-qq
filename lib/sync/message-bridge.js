...
export class MessageBridge {
  ...
  constructor({
    ...
    // Interactive New Session State per user: userOpenid -> { currentPath, page }
  ...
  async handleInboundC2C(data) {
    ...
    // Process slash command or user prompt
    if (content.startsWith('/')) {
      await this.handleCommand(authorOpenid, content, msgId);
    } else {
      await this.handleUserPrompt(authorOpenid, content, msgId);
    }
  }
  ...
  async handleInteraction(data) {
    ...
    const buttonData = data.data?.resolved?.button_data || '';
    const interactionId = data.id;

    this.logger.info?.(`[MessageBridge] Button interaction from ${authorOpenid}: ${buttonData} (interactionId: ${interactionId})`);
    ...
    // 2. Dispatch command (do not use interactionId as msg_id, pass null to use safe active message)
    if (buttonData.startsWith('/')) {
      await this.handleCommand(authorOpenid, buttonData, null);
    }
  }
  ...
  async cmdNewSession(userOpenid, args, msgId) {
    ...
    if (subAction === 'browse') {
      ...
      try {
        ...
        this.userNewSessionState.set(userOpenid, { currentPath: result.currentPath, page: 1 });
  ...
  /**
   * Handle ordinary user prompt from QQ (forward to DSH session)
   * @param {string} userOpenid - User OpenID
   * @param {string} content - Prompt text
   * @param {string} msgId - Incoming message ID
   */
  async handleUserPrompt(userOpenid, content, msgId) {
    ...
    this.logger.info?.(`[MessageBridge] Admitting prompt to session "${activeId}" (mode: ${mode})`);
    ...
    try {
      if (sessionController?.prompt) {
        ...
        liveAgent.followup({
          content: [{ type: 'text', text: content }],
          source: { kind: 'plugin', plugin: 'dsh-plugin-adapter-qq', rpcId },
        });
      } else {
  ...
  async onDshUserMessage(userOpenid, event) {
    ...
    // Filter out internal system injections, mnemon snapshots, runtime memory, prompt context, etc.
    if (
      fullText.startsWith('MNEMON RUNTIME MEMORY SNAPSHOT') ||
      fullText.includes('<system-reminder>') ||
      fullText.startsWith('The approval policy changed') ||
      fullText.startsWith('Current runtime context') ||
      fullText.startsWith('[sandbox:') ||
      fullText.startsWith('A skill is a reusable set of task-specific instructions') ||
      fullText.includes('Contents of USER.md') ||
      fullText.includes('Contents of MEMORY.md')
    ) {
      return;
    }

    this.logger.info?.(`[MessageBridge] Synchronizing Web UI question to QQ: "${fullText.slice(0, 30)}..."`);
    try {
      await this.apiClient.sendC2CMessage(userOpenid, {
